...
export default {
  ...
  methods:{
    ...
    adjustPageNumberList:function (pageNow) {
      if (this.pageNum >= 7){//只有最大页码大于等于7才会出现这种情况
        if (pageNow === 1 || pageNow === 2 || pageNow === 3){
          this.pageNumberList = [1,2,3,4,5,'...',this.pageNum];
        } else if (pageNow === this.pageNum - 2 || pageNow === this.pageNum - 1 || pageNow === this.pageNum){
          this.pageNumberList = [1,'...',this.pageNum-4,this.pageNum-3,this.pageNum-2,this.pageNum-1,this.pageNum];
        } else {
          this.pageNumberList = [1,'...',pageNow-1,pageNow,pageNow+1,'...',this.pageNum];
        }
      }
    },
    //选择页码
